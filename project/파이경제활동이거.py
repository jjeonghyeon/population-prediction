...
def plot_pie_chart(actual_population, predicted_population, category):
    labels = ['Actual', 'Predicted']
    sizes = [actual_population, predicted_population]

    fig, ax = plt.subplots()
    ax.pie(sizes, labels=labels, autopct=lambda p: '{:.1f}%'.format(p) if p > 0 else '', startangle=90)
    ax.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.title(f'{category} 인구 분포')
    plt.show()
...
def main():
    ...
    for category in df.columns[1:]:
        ...
        # 파이 차트 시각화
        actual_population = df[category].iloc[-1]
        plot_pie_chart(actual_population, future_population[-1], category)
